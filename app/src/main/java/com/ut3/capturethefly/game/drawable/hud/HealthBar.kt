...
class HealthBar(screenSize : RectF, private val currentHealth : () -> Float) : Drawable, Entity {
    ...
    override val rect = ImmutableRect(
        left = screenSize.left + padding,
        top = screenSize.top + padding,
        right = screenSize.left + width + padding,
    ...
    override fun update(delta: Float) {
        ...
        if (currentHealth() != percentageFilled) {
            changeCharge(currentHealth())
        }
    }
